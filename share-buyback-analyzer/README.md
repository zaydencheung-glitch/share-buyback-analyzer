...
1. Retrieve up to the four most recent completed fiscal years of
   **Diluted Average Shares** (and, where available, Basic Average
   Shares, repurchase spending, and stock-based compensation) via
   `yfinance`.
2. Clean the data: sort by fiscal year, remove exact duplicate rows,
   and preserve the original retrieved values unchanged alongside any
   derived figures.
3. Validate the data (see Section 8).
4. For each pair of *consecutive* fiscal years, compute **Net Shares
   Retired** and **Net Share Reduction Yield %** (Section 7). If a
   fiscal year is missing (e.g., 2021 and 2023 are both present but 2022
   is not), the program does **not** bridge the gap and compare 2023
   against 2021 as if it were a one-year change — that comparison is
   skipped entirely and logged as a data-quality note, since it would
   otherwise misrepresent a two-year change as a one-year yield.
5. Classify each year as **Net Share Reduction**, **Net Dilution**, or
   **Flat / Neutral**.
6. Aggregate across all available years for that company: average
   annual yield, cumulative percentage share-count change, count of
   reduction/dilution/neutral years, best year, worst year, and an
   overall classification.
7. Optionally compute a **Project Buyback Effectiveness Score** (Section
   9) — a transparent, project-defined heuristic, not an industry
   standard.
8. Repeat for all ten companies, then rank them in a summary table.
9. Generate charts and CSV outputs from the actual processed dataset.
...
output CSVs carries a `Data Source` / `data_source` field. Anything
...
`_overall_classification` docstring in `buyback_analyzer.py` if you're
reading the source.
...
After running `python buyback_analyzer.py`, look at:
- `data/processed/net_buyback_yield_all_companies.csv` — every company's
  year-by-year table.
- `data/processed/company_summary_ranking.csv` — the cross-company
  ranking and aggregate metrics.
- `reports/research_report.md` — fill in this template with your actual
...
1. `01_net_share_reduction_yield_by_company.png` — average annual net share reduction yield, by company.
2. `02_net_share_reduction_yield_over_time.png` — net share reduction yield trend, one line per company.
3. `03_cumulative_share_change_by_company.png` — cumulative diluted share-count change, by company.
4. `04_years_reduction_vs_dilution.png` — count of reduction/dilution/neutral years, by company.
5. `05_effectiveness_score_by_company.png` — Project Buyback Effectiveness Score, by company (only generated if the score could be computed for at least one company).
...
2. Retrieve data for each ticker in `TICKERS` (see the CONFIGURATION
   section at the top of `buyback_analyzer.py` — edit that list to
...
**A note on this delivered copy:** the code in this repository was
developed and unit-tested against synthetic data in a sandboxed
environment with no live internet access, so the `yfinance`-fetching
code path itself has not yet been exercised against the real, live API.
The very first time you run `python buyback_analyzer.py` on your own
machine, read the console output carefully — if a company is flagged
with a field-not-found warning, that means yfinance's schema for that
statement differs from what's coded here, and you'll need to either (a)
inspect the raw statement (see the command in the module docstring at
the top of `buyback_analyzer.py`) and add the correct field name to the
appropriate `CANDIDATE_FIELDS_*` list, or (b) enter the verified figure
manually and mark its source as `Manual Verification`.

## 14. File Structure

```
share-buyback-analyzer/
├── buyback_analyzer.py          # Main program (all core functions)
...
├── reports/
│   └── research_report.md       # Research report template (fill in with real output)
└── tests/
    └── test_buyback_analyzer.py # pytest test suite
```
